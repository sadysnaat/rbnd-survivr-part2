...
class TestJury < Minitest::Test
  ...
  def test_cast_votes_voting_seems_random
    @jury.members = @jury_members
    results = []
    # Assuming that 1/2^100 is the probability that a single player always wins
    # There is a very slight chance that jury selected on;y one player
    100.times do
      results << @jury.announce_winner(@jury.cast_votes(@finalists))
    end
    assert_includes results, @finalists.first
    assert_includes results, @finalists.last
